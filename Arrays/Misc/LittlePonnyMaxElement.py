...
"""
def LittlePonnyMaximumElement(A,B):
    min_ops = -1
    """
    Given - Can't set any element of the array equal to -1. B is given 
    Action/Goal - minimum number of operations required such that the maximum element is B.
    """
    n = len(A)
    if B not in A:
        return -1
    else:
        count_bigger_than_b = 0
        for i in range(n):
            if A[i] > B:
                count_bigger_than_b += 1

        return count_bigger_than_b

A = [2, 4, 3, 1, 5]
B = 3
A = [1, 4, 2]
B = 3
print(LittlePonnyMaximumElement(A,B))